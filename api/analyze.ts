...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log('🔷 PLUS Analysis - Using Gemini 2.0 Pro Exp');
    
    const { GoogleGenAI } = await import('@google/genai');
    const ai = new GoogleGenAI({ apiKey: geminiKey });
    
    // Build parts with PDF extracted text
    ...
    if (extractedTexts.length > 0) {
      parts.unshift({
        text: `📋 PDF থেকে সরাসরি নেওয়া টেক্সট:\n\n${extractedTexts.join('\n\n')}\n\n---\n\nএই টেক্সট PDF থেকে সরাসরি extract করা।`
      });
    }
...
JSON ফরম্যাটে উত্তর দিন:
{
  ...
  "nextSteps": ["পরবর্তী পদক্ষেপ"]
    ...
    // Try Gemini 2.0 Pro Exp
    let result: any = null;
    
    try {
      console.log('🤖 PLUS: Trying Gemini 2.0 Pro Exp...');
      ...
      console.log('✅ Gemini 2.0 Pro Exp responded');
      ...
      if (!openaiKey) {
        throw new Error('Gemini failed and no OpenAI key for fallback');
      }
      
      console.log('🔄 Falling back to GPT-5.1...');
