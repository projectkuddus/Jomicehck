      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-200 no-print">
        <div>
          <div className="flex items-center gap-3">
            ...
            {isPro && (
              <span className="flex items-center gap-1.5 px-3 py-1 bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-sm font-bold rounded-full shadow-lg">
                <Crown size={14} />
                PRO
              </span>
            )}
          ...
          <p className="text-sm text-slate-500 mt-1">
            Generated by JomiCheck AI • {isPro ? 'Premium Deep Analysis' : 'Standard Analysis'}
          </p>
        </div>
        <button 
          onClick={handlePrint}
        ...
        {isPro && report.pageByPageAnalysis && report.pageByPageAnalysis.length > 0 && (
          <div className="mb-8 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <button 
              ...
              className="w-full px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-gradient-to-r from-indigo-50 to-blue-50 hover:from-indigo-100 hover:to-blue-100 transition-colors"
            >
              <div className="flex items-center gap-3">
                <div className="p-2 bg-indigo-100 rounded-lg">
                  <FileText size={20} className="text-indigo-600" />
                </div>
                <h3 className="text-xl font-bold text-slate-800">Page-by-Page Analysis ({report.pageByPageAnalysis.length} pages)</h3>
                <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-xs font-bold rounded">PRO</span>
              </div>
              {expandedSections.pageAnalysis ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
            </button>
                  ...
                  <div key={idx} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-slate-800 bangla-text">{page.pageType}</span>
                      </div>
                      {page.readabilityScore !== undefined && (
                        <span className={`text-xs px-2 py-1 rounded ${
                          page.readabilityScore > 70 ? 'bg-green-100 text-green-700' :
                          page.readabilityScore > 40 ? 'bg-yellow-100 text-yellow-700' :
                          'bg-red-100 text-red-700'
                        }`}>
                          Readability: {page.readabilityScore}%
                        </span>
                      )}
                    </div>
